...
public:
  ...
  template <typename S> inline Vector3 operator*(const S &c) const {
...
public:
  ...
  struct Face {
    ...
    Face()
        : m_he(std::numeric_limits<size_t>::max()), m_P(),
  ...
  getVertexIndicesOfHalfEdge(const HalfEdge &he) const {
...
public:
  ...
  inline void
  writeWaveformOBJ(const std::string &filename,
...
template <typename T> inline T defaultEps() { return T{1} / T{10000000}; }
